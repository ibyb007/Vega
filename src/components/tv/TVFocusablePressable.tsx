...
interface TVFocusablePressableProps extends Omit<PressableProps, 'style' | 'children'> {
  ...
  // Android-only native focus-search overrides. Take a view's native node
  // handle (from `findNodeHandle`) -- not declared on `PressableProps` in
  // every RN type-defs version, so they're spelled out explicitly here
  // rather than relying on `...rest` picking them up untyped.
